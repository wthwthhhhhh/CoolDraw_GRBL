...
enum class NonModal : uint8_t {
    ...
    SetMachinePosition = 202,  //G92.2 (Do not alter value)
};
...
typedef struct {
    ...
    float position[MAX_N_AXIS];//坐标 解释器认为工具在代码中的位置Where the interpreter considers the tool to be at this point in the code


    float coord_system[MAX_N_AXIS];  // 当前工作坐标系(G54+)。从绝对机存储偏移量Current work coordinate system (G54+). Stores offset from absolute machine
    // position in mm. Loaded from non-volatile storage when called.
    float coord_offset[MAX_N_AXIS];//保留G92坐标偏移量(工作坐标)相对于
//机器零位单位为mm。重置和引导时清除。
    float tool_length_offset;  //跟踪工具长度偏移值时启用。 Tracks tool length offset value when enabled.
} parser_state_t;
