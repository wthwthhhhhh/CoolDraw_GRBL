...
//设置应用工具长度偏移量的轴。假设主轴总是平行于
//所选轴与工具朝向负方向。换句话说，这是积极的
//从当前位置减去工具长度偏移值。
const int TOOL_LENGTH_OFFSET_AXIS = Z_AXIS;  // Default z-axis. Valid values are X_AXIS, Y_AXIS, or Z_AXIS.

//启用此功能后，Grbl发送回它接收到的已预先解析的行(空格
//删除，大写字母，无评论)，并由Grbl立即执行。回声将不会
//在行缓冲区溢出时发送，但应该为所有正常行发送到Grbl。例如，如果一个用户
//发送'g1 x1.032 y2.45 (test comment)'， Grbl将以'[echo: G1X1.032Y2.45]'的形式返回。
//只回显GCode行，不回显以$或[ESP]开头的命令行。
//注意:只用于调试目的!!当进行响应时，这占用了宝贵的资源，并会产生影响
//性能。如果绝对需要正常的操作，串行写缓冲区应该大大增加
//帮助最小化串行写协议中的传输等待。
...
#define FORCE_BUFFER_SYNC_DURING_NVS_WRITE//默认启用。注释以禁用。
//在Grbl v0.9和更早的版本中，有一个“WPos:”工作位置报告的旧bug
//可能与正在执行的内容无关，因为' WPos: '是基于GCode解析器状态的，这是什么
//可几个动作后。此选项强制计划器缓冲区清空、同步和停止
//每当有命令改变工作坐标偏移' G10,G43.1,G92,G54-59 '时，运动。
//这是确保' WPos: '始终正确的最简单方法。幸运的是，这种情况非常罕见
//使用任何这些命令都需要通过它们进行连续的移动。
#define FORCE_BUFFER_SYNC_DURING_WCO_CHANGE//默认启用。注释以禁用。
//默认情况下，Grbl禁用所有G38的提要速率覆盖。X探测周期命令。虽然这
//可能与一些专业级别的机器控制不同，它应该是这样的，这是有争议的。
//大多数探头传感器产生不同程度的误差取决于速度。通过
//保持探测周期与它们的程序馈电速率一致，探测传感器应该更多
//可重复的。如果需要，可以通过取消注释下面的定义来禁用此行为。
//# ALLOW_FEED_OVERRIDE_DURING_PROBE_CYCLES默认禁用。取消注释以启用。
//在安全门状态下启用和配置停车运动方法。主要为oem服务
//他们希望他们的集成机器具有这个功能。目前，Grbl的假设是
//尽管编写了停车实现，但停车运动只涉及一个轴
//通过改变停车位置，可以很容易地重构任意数量的不同轴上的运动
//源代码。此时，Grbl只支持停放一个轴(通常是z轴)
//在后退时向正方向移动，在恢复位置时向负方向移动。
//该运动执行与缓慢拉出收回运动，断电，和快速停车。
//恢复到恢复位置遵循这些反向的设置动作:快速恢复到
//拉出位置，加电并暂停，然后在
//拉出速度较慢。
