...
//将轴值格式化为字符串并在RPT中返回该字符串
//注意:rpt至少应该有size: axesStringLen
static void report_util_axis_values(float* axis_value, char* rpt) {
    uint8_t     idx;
    char        axisVal[coordStringLen];
    float       unit_conv = 1.0;//单位转换倍率默认为mm
    const char* format    = "%4.3f";//默认-报告mm到小数点后3位
    rpt[0]                = '\0';
    if (report_inches->get()) {
        unit_conv = 1.0 / MM_PER_INCH;
        format    = "%4.4f";//报告英寸到小数点后4位
    }
...
//计算状态报告的位置。
//Float print_position =返回的位置
//Float wco =返回工作坐标偏移量
//对于工作位置补偿，Bool wpos = true
//打印实时数据。这个函数获取步进子程序的实时快照
//以及数控机床的实际位置。用户可以将以下功能更改为
//具体需要，但所需的实时数据报告必须尽可能短。这是
//需要，因为它最小化计算开销，并允许GRBL保持平稳运行，
//特别是在g代码程序的快速，短线段和高频报告(5-20Hz)。
