...
#    endif
    ...
    void Web_Server::_handle_web_command(bool silent) {
        ...
        if (ESPpos > -1) {
            ...
            bool hasError =false;
            uint8_t     sindex = 0;
// TODO设置-这是非常低效的。get_Splited_Value()是O(n^2)
//当它可以很容易地为O(n)。而且，推它也很容易
//将整个字符串放入Serial2Socket中，并从那里拉出行。
            for (uint8_t sindex = 0; (scmd = get_Splited_Value(cmd, '\n', sindex)) != ""; sindex++) {
// 0xC2是HTML编码前缀，在UTF-8模式下，
//在0x90和0xa0-0bf之前，都是GRBL实时命令。
//对于0x91-0x9f还有其他编码，所以我不确定
//这些命令如何或是否工作。
