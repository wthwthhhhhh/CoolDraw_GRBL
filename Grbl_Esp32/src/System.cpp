...
// Returns if safety door is ajar(T) or closed(F), based on pin state.根据销钉状态，如果安全门半开(T)或关闭(F)，则返回。
...
float* system_get_mpos() {
    ...
    //                MsgLevel::Warning,
    //                "x_axis_settings max_travel:%f, home_mpos:%f, run_current:%f, hold_current:%f",
    //                x_axis_settings->max_travel->get(),
    //                x_axis_settings->home_mpos->get(),
    //                x_axis_settings->run_current->get(),
    //                x_axis_settings->hold_current->get());
    //   grbl_msg_sendf(CLIENT_SERIAL,
    //                MsgLevel::Warning,
