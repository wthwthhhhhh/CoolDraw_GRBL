...
//执行绝对毫米坐标的线性运动。以毫米/秒为单位的进给速度

//除非invert_feed_rate为true。然后feed_rate意味着运动应该在

//(1分钟)/feed_rate时间。

//注意:这是到grbl计划器的主网关。所有直线运动，包括弧线

//段，在传递给计划器之前必须通过这个例程。的分离

// Mc_line和plan_buffer_line主要用于放置非规划者类型的函数

//在计划表中并让反弹补偿或罐头循环集成简单直接。

//如果行被提交给规划者，则返回true;如果行被故意丢弃，则返回false。
bool mc_line(float* target, plan_line_data_t* pl_data) {
    ...
    do {
        ...
        if (sys.abort) {
            sys_pl_data_inflight = NULL;
            return submitted_result;//如果系统中止，则退出。
        }
        if (plan_check_full_buffer()) {
            protocol_auto_cycle_start();  // Auto-cycle 当缓冲区满时启动。
        } else {
            break;
        }
    } while (1);
//计划和队列运动到计划缓冲区
// uint8_t plan_status;//正常操作时不使用。
...
//执行offset格式的arc位置==当前xyz，目标==目标xyz，

// offset ==当前xyz的偏移量，axis_X在工具空间中定义圆平面，axis_linear为

//螺旋移动的方向，半径==圆半径，顺时针布尔值。使用

//表示向量变换方向。

//弧线是由大量微小的线段构成的。弦耐受力

//在arc_tolerance设置中配置，它被定义为最大法线
...
void mc_arc(float*            target,
    ...
    if (segments) {
        ...
        /* Vector rotation by transformation matrix: r is the original vector, r_T is the rotated vector,
           ...
           对于圆弧的产生，圆心是旋转轴，半径向量是
从圆心到初始位置定义。每一线段由连续线段构成
矢量旋转。单个精度值累积的误差在极少数情况下会大于刀具精度
用例。实现了精确的弧线路径校正。这种方法避免了过多的问题
昂贵的三角运算[sin()，cos()，tan()]，每个运算需要100-200 usec。

小角度近似可以进一步减少计算开销。三阶近似
(二阶sin()有太多的误差)适用于大多数，如果不是所有的CNC应用程序。注意这一点
当theta_per_segment大于时，近似值将开始累积数值漂移误差
~0.25 rad(14°)和近似连续使用，没有校正几十次。这
这种情况极不可能发生，因为段长度和theta_per_segment是自动生成的
并按弧度公差设置进行缩放。只有一个非常大的弧度公差设置，对数控不现实
应用时，会造成此数值漂移误差。但是，最好从a设置N_ARC_CORRECTION
低的~4到高的~20左右，以避免三角运算，同时保持电弧生成的准确性。

这种近似还允许mc_arc立即在规划器中插入线段
没有计算cos()或sin()的初始开销。在需要使用电弧的时候
更正一下，计划器应该已经赶上了由初始Mc_arc开销引起的延迟。
当有连续的弧运动时，这是很重要的。
        */
...
//规划并执行停车单专项运动案例。独立于主规划器缓冲区。
//注意:使用始终空闲的计划器环形缓冲头来存储执行的运动参数。
