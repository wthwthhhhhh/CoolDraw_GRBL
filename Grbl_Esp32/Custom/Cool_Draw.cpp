...
void machine_init() {
    ...
    //计算线缆A长度
    float A = sqrt((rivetA[X_AXIS] - MPosition[X_AXIS]) * (rivetA[X_AXIS] - MPosition[X_AXIS]) +
                   (rivetA[Y_AXIS] - MPosition[Y_AXIS]) * (rivetA[Y_AXIS] - MPosition[Y_AXIS]));  //- (rivetB[X_AXIS] / 2);
    //计算线缆B长度
    float B = sqrt((rivetB[X_AXIS] - MPosition[X_AXIS]) * (rivetB[X_AXIS] - MPosition[X_AXIS]) +
                   (rivetB[Y_AXIS] - MPosition[Y_AXIS]) * (rivetB[Y_AXIS] - MPosition[Y_AXIS]));  //- (rivetB[X_AXIS] / 2);
...
bool cartesian_to_motors(float* target, plan_line_data_t* pl_data, float* position) {
    ...
    motors[X_AXIS] = sqrt((rivetA[X_AXIS] - target[X_AXIS]) * (rivetA[X_AXIS] - target[X_AXIS]) +
                          (rivetA[Y_AXIS] - target[Y_AXIS]) * (rivetA[Y_AXIS] - target[Y_AXIS]));  //- (rivetB[X_AXIS] / 2);
    motors[Y_AXIS] = sqrt((rivetB[X_AXIS] - target[X_AXIS]) * (rivetB[X_AXIS] - target[X_AXIS]) +
                          (rivetB[Y_AXIS] - target[Y_AXIS]) * (rivetB[Y_AXIS] - target[Y_AXIS]));  //- (rivetB[X_AXIS] / 2);

    motors[Z_AXIS] = target[Z_AXIS];
    motors[X_AXIS] = isnan(motors[X_AXIS]) ? 0 : motors[X_AXIS];
    motors[Y_AXIS] = isnan(motors[Y_AXIS]) ? 0 : motors[Y_AXIS];

    return mc_line(motors, pl_data);
    }
    catch(const std::exception& e)
    {
        std::cerr << e.what() << '_cartesian_to_motors \n';
        return false;
    }
}
...
void motors_to_cartesian(float* cartesian, float* motor, int n_axis) {
    ...
    float A = motor[X_AXIS];
    float B = motor[Y_AXIS];
    float MPosition[MAX_N_AXIS];
    //10000-
    ...
    {
        std::cerr << e.what() << '_motors_to_cartesian \n';
    }
}
